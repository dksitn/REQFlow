...
export default function ProjectEvaluationPage() {
  ...
  const [images, setImages] = useState<Record<string, string>>({}); // 存放 AS-IS / TO-BE 圖片網址
  ...
  const handleEdit = async (field: string, currentValue: string) => {
    if (locks[field] && locks[field].user_id !== currentUser?.id) return; // 被別人鎖了
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, type: 'AS-IS' | 'TO-BE') => {
    ...
    setImages(prev => ({ ...prev, [type]: fakeUrl }));
  };
  ...
  // 輸出 PDF (呼叫瀏覽器列印，依賴 print CSS 控制分頁)
  ...
  const GridBlock = ({ title, dbField, type = 'textarea' }: { title: string, dbField: string, type?: 'textarea' | 'image' }) => {
    ...
    return (
      <div className={`flex flex-col bg-white border ${isCompleted ? 'border-emerald-200 shadow-emerald-50' : isEditing ? 'border-indigo-300 shadow-md ring-1 ring-indigo-500' : 'border-slate-200'} rounded-xl shadow-sm overflow-hidden transition-all h-full break-inside-avoid`}>
        <div className={`px-4 py-3 flex items-center justify-between border-b ${isCompleted ? 'bg-emerald-50 border-emerald-100' : 'bg-slate-50 border-slate-100'}`}>
          <div className="flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${isCompleted ? 'bg-emerald-500' : isLockedByOther ? 'bg-rose-500' : isEditing ? 'bg-indigo-500 animate-pulse' : 'bg-amber-400'}`}></span>
        ...
        <div className="p-4 flex-1 flex flex-col gap-3">
          {isEditing ? (
            ...
            <div className={`flex-1 text-sm whitespace-pre-wrap ${value ? 'text-slate-700' : 'text-slate-400 italic'}`}>
              {type === 'image' && images[dbField] ? <img src={images[dbField]} className="max-w-full h-auto rounded-lg" alt={title}/> : value || '尚未填寫資料...'}
            </div>
          )}
          ...
          <div className="print:hidden">
            ...
            {!isEditing && !isCompleted && value && !isLockedByOther && (
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col font-sans pb-24 print:bg-white print:pb-0">
      ...
      {/* 🚀 第一層格子：頂部固定資訊列 */}
      <div className="sticky top-0 z-40 bg-white border-b border-slate-200 shadow-sm print:static print:shadow-none print:border-none">
      ...
      <div className="max-w-[1600px] mx-auto w-full px-6 py-8 flex flex-col gap-8 print:p-0 print:gap-4 print:w-full">
        
        {/* PDF 第一頁內容 (第一到第四層) */}
        <div className="print:break-after-page">
          
          {/* 🚀 第二層格子：專案負責人 */}
          <section className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 flex flex-col gap-4 mb-8 print:border-none print:shadow-none print:p-0">
        ...
        {/* PDF 第二頁內容 (第五層 AS-IS / TO-BE) */}
        <div className="print:break-after-page print:pt-8">
          {/* 🚀 第五層格子：圖片上傳 */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex flex-col bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden h-[800px]">
              <div className="px-4 py-3 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
                <h3 className="text-sm font-black text-slate-800">現行系統架構 (AS-IS)</h3>
                <label className="print:hidden cursor-pointer text-xs font-bold text-indigo-600 hover:bg-indigo-50 px-2 py-1 rounded">
                  上傳圖片
                  <input type="file" accept="image/*" className="hidden" onChange={(e) => handleImageUpload(e, 'AS-IS')} />
                </label>
              </div>
              <div className="p-4 flex-1 flex items-center justify-center bg-slate-50/50">
                {images['AS-IS'] ? <img src={images['AS-IS']} className="max-w-full max-h-full object-contain" alt="AS-IS"/> : <span className="text-slate-400 font-bold text-sm">尚未上傳圖片</span>}
              </div>
            </div>

            <div className="flex flex-col bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden h-[800px]">
              <div className="px-4 py-3 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
                <h3 className="text-sm font-black text-slate-800">未來系統架構 (TO-BE)</h3>
                <label className="print:hidden cursor-pointer text-xs font-bold text-indigo-600 hover:bg-indigo-50 px-2 py-1 rounded">
                  上傳圖片
                  <input type="file" accept="image/*" className="hidden" onChange={(e) => handleImageUpload(e, 'TO-BE')} />
                </label>
              </div>
              <div className="p-4 flex-1 flex items-center justify-center bg-slate-50/50">
                {images['TO-BE'] ? <img src={images['TO-BE']} className="max-w-full max-h-full object-contain" alt="TO-BE"/> : <span className="text-slate-400 font-bold text-sm">尚未上傳圖片</span>}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="print:pt-8">
          ...
          {/* 🚀 簽核流程格子 (PDF 專用顯示或畫面最下方顯示) */}
          <section className="bg-white rounded-2xl p-6 border border-slate-200 print:border-slate-800 print:rounded-none mt-12">
            <h2 className="text-base font-black text-slate-800 mb-4 print:text-center">專案簽核流程</h2>
            <div className="grid grid-cols-3 gap-0 border-t border-l border-slate-800">
              {['需求單位經辦', '需求單位科主管', '需求單位主管', '智慧金融處經辦', '智慧金融處科主管', '智慧金融處主管'].map((role, idx) => (
                <div key={idx} className="border-r border-b border-slate-800 h-32 flex flex-col">
                  <div className="bg-slate-100 border-b border-slate-800 p-2 text-center text-xs font-bold text-slate-800">{role}</div>
      ...
      {/* Admin 解除鎖定按鈕 (只有 Admin 且有鎖定資料時顯示) */}
