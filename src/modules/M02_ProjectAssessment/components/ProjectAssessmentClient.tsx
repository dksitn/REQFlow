...
function EditableCard({ 
  ...
  useEffect(() => {
    const checkLock = async () => {
      const { data } = await supabase.from('m01_edit_locks').select('locked_by, locked_at, m01_users(full_name)').eq('project_id', projectId).eq('field_name', fieldKey).maybeSingle();
      if (data) {
        const lockedTime = new Date(data.locked_at).getTime();
        if (Date.now() - lockedTime > 30 * 60 * 1000) {
          ...
          setLockOwnerName((Array.isArray(usersData) ? usersData[0]?.full_name : usersData?.full_name) || '其他同事'); 
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      if (existingLock && (Date.now() - new Date(existingLock.locked_at).getTime() < 30 * 60 * 1000) && existingLock.locked_by !== currentUserId) {
        return alert('此區塊剛被他人鎖定，無法編輯。');
      }
      await supabase.from('m01_edit_locks').upsert({ project_id: projectId, field_name: fieldKey, locked_by: currentUserId, locked_at: new Date().toISOString() }, { onConflict: 'project_id,field_name' });
      setIsEditing(true);
  ...
  const handleSave = async () => {
    setIsLoading(true);
    try { 
      await onSave(fieldKey, text); setIsEditing(false); 
      if (currentUserId) await supabase.from('m01_edit_locks').delete().eq('project_id', projectId).eq('field_name', fieldKey);
    ...
    <div className={`${styles.bg} p-6 rounded-2xl border shadow-sm flex flex-col transition-all ${minHeight} ${isEditing ? 'ring-2 ring-blue-500/20 border-blue-400' : `${styles.border} ${styles.hover} hover:shadow-md`}`}>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          {!isEditing && isConfirmed && <span className="text-[10px] bg-white text-emerald-600 px-2 py-0.5 rounded font-extrabold flex items-center gap-1 shadow-sm border border-emerald-100"><Check className="w-3 h-3"/> 已確認</span>}
          {!isEditing && isLockedByOther && <span className="flex items-center gap-1 text-[10px] font-bold bg-rose-50 text-rose-600 px-2 py-1 rounded"><Lock className="w-3 h-3" /> {lockOwnerName} 編輯中</span>}
        </div>
      </div>
        ...
        <div className="flex-1 flex flex-col animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="flex justify-between items-center mt-4">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={handleSave} disabled={isLoading} className="flex items-center gap-1.5 px-4 py-1.5 bg-[#3B82F6] text-white text-xs font-bold rounded-lg hover:bg-blue-600 transition-colors shadow-sm">{isLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Save className="w-3.5 h-3.5" />} 儲存</button>
            </div>
          </div>
        </div>
      ) : (
        <div className="flex-1 flex flex-col justify-between group h-full">
          <div className={`text-xs leading-relaxed overflow-y-auto mb-4 font-medium custom-scrollbar ${isLockedByOther ? 'text-slate-400 cursor-not-allowed' : 'text-slate-700'}`}>
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <div className="p-4 flex-1 overflow-y-auto bg-white space-y-2 custom-scrollbar">
          {isLoading ? <div className="py-8 flex justify-center"><Loader2 className="w-6 h-6 animate-spin text-blue-500" /></div> : departments.map(dept => (
            <div key={dept.id} onClick={() => setSelected(dept.name)} className={`px-4 py-3 rounded-xl border cursor-pointer font-bold text-sm transition-all flex items-center justify-between group ${selected === dept.name ? 'bg-blue-50 border-blue-400 text-blue-700 shadow-sm ring-2 ring-blue-500/20' : 'bg-white border-slate-200 text-slate-600 hover:border-blue-300 hover:bg-slate-50'}`}>
              {dept.name} <div className={`w-5 h-5 rounded-full border flex items-center justify-center transition-colors ${selected === dept.name ? 'bg-blue-600 border-blue-600' : 'border-slate-300 group-hover:border-blue-400'}`}>{selected === dept.name && <Check className="w-3 h-3 text-white" />}</div>
            </div>
          ))}
        </div>
...
export default function ProjectAssessmentPage() {
  ...
  const handleConfirmField = async (fieldKey: string) => {
    ...
    setProject({ ...project, confirmed_fields: updatedConfirmed });
  };

  const handleSaveText = async (fieldKey: string, newText: string) => {
    if (isReadOnlyMember) return;
    const updatedConfirmed = { ...(project.confirmed_fields || {}), [fieldKey]: false };
    await supabase.from('m01_projects').update({ [fieldKey]: newText, confirmed_fields: updatedConfirmed }).eq('id', projectId);
    setProject((prev: any) => ({ ...prev, [fieldKey]: newText, confirmed_fields: updatedConfirmed }));
  };
  ...
  const toggleSelection = (name: string) => { setTempSelections(prev => prev.includes(name) ? prev.filter(n => n !== name) : [...prev, name]); };
    ...
    <div className="absolute inset-0 overflow-y-auto bg-[#F8FAFC] flex flex-col font-sans scroll-smooth custom-scrollbar">
      ...
      <div className="px-8 pt-8 pb-32 max-w-[1400px] mx-auto w-full flex flex-col gap-6 shrink-0">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 w-32"><div className="w-full bg-slate-200 rounded-full h-2 overflow-hidden"><div className="bg-emerald-500 h-2 rounded-full transition-all duration-1000 ease-out" style={{ width: `${completenessPercent}%` }}></div></div><span className="text-xs font-black text-slate-600">{completenessPercent}%</span></div>
          </div>
        </div>

        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
          <div><p className="text-[11px] font-bold text-slate-400 mb-2">專案編號</p><p className="text-sm font-black text-slate-500 font-mono bg-slate-50 px-3 py-2.5 rounded-xl border border-slate-100">{project.project_code}</p></div>
          <div>
            ...
            <button onClick={() => !isReadOnlyMember && setIsDeptModalOpen(true)} className={`w-full flex items-center justify-between px-4 py-2.5 bg-slate-50 border border-slate-100 rounded-xl transition-all group ${isReadOnlyMember ? 'cursor-not-allowed opacity-80' : 'hover:bg-white hover:border-blue-300'}`}>
              <div className="flex items-center gap-2"><Building2 className={`w-4 h-4 text-slate-500 ${!isReadOnlyMember && 'group-hover:text-blue-500'}`} /><span className={`text-sm font-bold text-slate-700 ${!isReadOnlyMember && 'group-hover:text-blue-700'}`}>{project.department || '請選擇單位'}</span></div>
              {!isReadOnlyMember && <ChevronDown className="w-4 h-4 text-slate-400 group-hover:text-blue-400" />}
            </button>
          </div>
          <div className="md:col-span-2">
            <div className="flex items-center justify-between"><p className="text-[11px] font-bold text-slate-400 mb-2 flex items-center gap-1">專案名稱 {!isReadOnlyMember && <Edit2 className="w-3 h-3 text-slate-300" />}</p><p className="text-[11px] font-bold text-slate-400 mb-2">專案狀態</p></div>
            <div className="flex items-center gap-4">
              ...
              <button onClick={() => !isReadOnlyMember && setIsStatusModalOpen(true)} className={`flex items-center justify-between gap-4 px-4 py-2.5 bg-white border border-slate-200 rounded-xl transition-all shrink-0 ${isReadOnlyMember ? 'cursor-not-allowed opacity-80' : 'hover:bg-blue-50 hover:border-blue-300'}`}>
                <span className="text-sm font-black text-[#3B82F6]">{project.status_name_snapshot}</span>{!isReadOnlyMember && <ChevronDown className="w-4 h-4 text-slate-400" />}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
          <h2 className="text-sm font-black text-slate-800 mb-4">專案負責人名單管理</h2>
                   ...
                   <div className="flex flex-wrap gap-2">
                     {members.map((member: string) => (
                       <span key={member} className={`inline-flex items-center gap-1.5 px-3 py-1.5 border rounded-lg text-xs font-bold shadow-sm ${team.bgClass}`}>
                         {member} {!isReadOnlyMember && <X onClick={() => handleRemoveMemberInline(team.key, member)} className="w-3 h-3 opacity-50 hover:opacity-100 cursor-pointer" />}
                       </span>
                     ))}
                   </div>
              ...
              <div className="flex-1 relative flex items-center justify-center flex-col">
                ...
                {isUploading.AS_IS ? <div className="flex flex-col items-center text-blue-500"><Loader2 className="w-8 h-8 animate-spin mb-2" /><span className="text-xs font-bold">上傳中...</span></div> : images.AS_IS ? (
                  <>
                    ...
                    {!isReadOnlyMember && !confirmedFields['image_as_is'] && <div className="absolute bottom-4 left-0 right-0 flex justify-center opacity-100 group-hover:opacity-0 transition-opacity"><button onClick={() => handleConfirmField('image_as_is')} className="flex items-center gap-1.5 px-4 py-2 bg-emerald-500 text-white text-xs font-extrabold rounded-full hover:bg-emerald-600 shadow-lg animate-bounce"><CheckCircle2 className="w-4 h-4" /> 確認圖片無誤</button></div>}
                  </>
                ) : <div onClick={() => !isReadOnlyMember && asIsInputRef.current?.click()} className={`flex flex-col items-center justify-center w-full h-full transition-all ${isReadOnlyMember ? 'text-slate-300 cursor-not-allowed' : 'text-slate-400 hover:text-blue-600 hover:bg-white cursor-pointer'}`}><UploadCloud className="w-8 h-8 mb-2" /><span className="text-sm font-bold">{isReadOnlyMember ? '尚無圖片' : '上傳 AS-IS 圖片'}</span></div>}
              </div>
            </div>
            <div className="relative border border-slate-200 rounded-xl overflow-hidden min-h-[260px] flex flex-col group bg-slate-50/50">
              <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
              ...
              <div className="flex-1 relative flex items-center justify-center flex-col">
                ...
                {isUploading.TO_BE ? <div className="flex flex-col items-center text-blue-500"><Loader2 className="w-8 h-8 animate-spin mb-2" /><span className="text-xs font-bold">上傳中...</span></div> : images.TO_BE ? (
                  <>
                    ...
                    {!isReadOnlyMember && !confirmedFields['image_to_be'] && <div className="absolute bottom-4 left-0 right-0 flex justify-center opacity-100 group-hover:opacity-0 transition-opacity"><button onClick={() => handleConfirmField('image_to_be')} className="flex items-center gap-1.5 px-4 py-2 bg-emerald-500 text-white text-xs font-extrabold rounded-full hover:bg-emerald-600 shadow-lg animate-bounce"><CheckCircle2 className="w-4 h-4" /> 確認圖片無誤</button></div>}
                  </>
                ) : <div onClick={() => !isReadOnlyMember && toBeInputRef.current?.click()} className={`flex flex-col items-center justify-center w-full h-full transition-all ${isReadOnlyMember ? 'text-slate-300 cursor-not-allowed' : 'text-slate-400 hover:text-blue-600 hover:bg-white cursor-pointer'}`}><UploadCloud className="w-8 h-8 mb-2" /><span className="text-sm font-bold">{isReadOnlyMember ? '尚無圖片' : '上傳 TO-BE 圖片'}</span></div>}
              </div>
            </div>
      ...
      {isStatusModalOpen && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col">
            ...
            <div className="p-4 bg-slate-50/50 space-y-2">{statusOptions.map((opt) => (<button key={opt} onClick={() => handleUpdateStatus(opt)} className={`w-full flex items-center justify-between p-3 border rounded-xl font-bold text-sm transition-all ${project.status_name_snapshot === opt ? 'border-blue-500 bg-blue-50 text-blue-700 ring-1 ring-blue-500/20' : 'bg-white border-slate-200 text-slate-700 hover:bg-slate-50'}`}>{opt}{project.status_name_snapshot === opt && <Check className="w-4 h-4 text-blue-600" />}</button>))}</div>
          </div>
        </div>
      )}
      ...
      {memberModalConfig?.isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col">
            ...
            <div className="px-5 py-4 border-t flex gap-3 bg-slate-50/50"><button onClick={() => setMemberModalConfig(null)} className="flex-1 py-2.5 text-xs font-bold text-slate-600 bg-white border rounded-xl hover:bg-slate-50">取消</button><button onClick={confirmAssignees} className="flex-1 py-2.5 text-xs font-bold text-white bg-blue-600 rounded-xl hover:bg-blue-700">確認名單 ({tempSelections.length})</button></div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[60] bg-slate-400/80 backdrop-blur-sm overflow-y-auto animate-in fade-in duration-200 print:bg-white print:p-0">
          ...
          <div className="flex flex-col items-center gap-12 py-12 print:block print:py-0 print:gap-0">
            <article className="w-[297mm] h-[210mm] max-w-full bg-white shadow-2xl p-10 flex flex-col shrink-0 print:w-full print:h-screen print:border-none print:shadow-none print:p-0 print:m-0 print:break-after-page overflow-hidden">
              ...
              <div className="flex-1 grid grid-cols-2 gap-4 min-h-0"><div className="bg-slate-50/70 p-4 rounded-lg border border-slate-200 flex flex-col min-h-0"><h3 className="text-xs font-black text-slate-800 mb-2 border-b border-slate-200 pb-1.5 shrink-0">現行工作職掌與工作流程</h3><p className="text-[11px] text-slate-700 leading-relaxed whitespace-pre-wrap overflow-y-auto flex-1 font-medium">{project.workflow_text || '尚未填寫'}</p></div><div className="bg-slate-50/70 p-4 rounded-lg border border-slate-200 flex flex-col min-h-0"><h3 className="text-xs font-black text-slate-800 mb-2 border-b border-slate-200 pb-1.5 shrink-0">現行作業痛點</h3><p className="text-[11px] text-slate-700 leading-relaxed whitespace-pre-wrap overflow-y-auto flex-1 font-medium">{project.as_is_text || '尚未填寫'}</p></div></div>
              <div className="flex-1 grid grid-cols-3 gap-4 mt-4 min-h-0"><div className="bg-slate-50/70 p-4 rounded-lg border border-slate-200 flex flex-col min-h-0"><h3 className="text-xs font-black text-slate-800 mb-2 border-b border-slate-200 pb-1.5 shrink-0">影響範圍－人員</h3><p className="text-[11px] text-slate-700 leading-relaxed whitespace-pre-wrap overflow-y-auto flex-1 font-medium">{project.impact_people_text || '尚未填寫'}</p></div><div className="bg-slate-50/70 p-4 rounded-lg border border-slate-200 flex flex-col min-h-0"><h3 className="text-xs font-black text-slate-800 mb-2 border-b border-slate-200 pb-1.5 shrink-0">影響範圍－時間</h3><p className="text-[11px] text-slate-700 leading-relaxed whitespace-pre-wrap overflow-y-auto flex-1 font-medium">{project.impact_time_text || '尚未填寫'}</p></div><div className="bg-slate-50/70 p-4 rounded-lg border border-slate-200 flex flex-col min-h-0"><h3 className="text-xs font-black text-slate-800 mb-2 border-b border-slate-200 pb-1.5 shrink-0">影響範圍－效益</h3><p className="text-[11px] text-slate-700 leading-relaxed whitespace-pre-wrap overflow-y-auto flex-1 font-medium">{project.impact_benefit_text || '尚未填寫'}</p></div></div>
            </article>
            <article className="w-[297mm] h-[210mm] max-w-full bg-white shadow-2xl p-10 flex flex-col shrink-0 print:w-full print:h-screen print:border-none print:shadow-none print:p-0 print:m-0 print:break-after-page overflow-hidden">
            ...
            <article className="w-[297mm] h-[210mm] max-w-full bg-white shadow-2xl p-10 flex flex-col shrink-0 print:w-full print:h-screen print:border-none print:shadow-none print:p-0 print:m-0 overflow-hidden">
              ...
              <div className="flex-1 grid grid-cols-2 grid-rows-2 gap-6 min-h-0"><div className="bg-slate-50/70 p-4 rounded-lg border border-slate-200 flex flex-col min-h-0"><h3 className="text-xs font-black text-slate-800 mb-2 border-b border-slate-200 pb-1.5 shrink-0">業務評估 (Business Evaluation)</h3><p className="text-[11px] text-slate-700 leading-relaxed whitespace-pre-wrap overflow-y-auto flex-1 font-medium">{project.eval_business || '尚未填寫'}</p></div><div className="bg-slate-50/70 p-4 rounded-lg border border-slate-200 flex flex-col min-h-0"><h3 className="text-xs font-black text-slate-800 mb-2 border-b border-slate-200 pb-1.5 shrink-0">技術評估 (Technical Assessment)</h3><p className="text-[11px] text-slate-700 leading-relaxed whitespace-pre-wrap overflow-y-auto flex-1 font-medium">{project.eval_technical || '尚未填寫'}</p></div><div className="bg-slate-50/70 p-4 rounded-lg border border-slate-200 flex flex-col min-h-0"><h3 className="text-xs font-black text-slate-800 mb-2 border-b border-slate-200 pb-1.5 shrink-0">成效追蹤指標 (Tracking Metrics)</h3><p className="text-[11px] text-slate-700 leading-relaxed whitespace-pre-wrap overflow-y-auto flex-1 font-medium">{project.eval_kpi || '尚未填寫'}</p></div><div className="bg-slate-50/70 p-4 rounded-lg border border-slate-200 flex flex-col min-h-0"><h3 className="text-xs font-black text-slate-800 mb-2 border-b border-slate-200 pb-1.5 shrink-0">綜合評估 (Comprehensive Judgment)</h3><p className="text-[11px] text-slate-700 leading-relaxed whitespace-pre-wrap overflow-y-auto flex-1 font-medium">{project.eval_conclusion || '尚未填寫'}</p></div></div>
            </article>
          </div>
        </div>
      )}
